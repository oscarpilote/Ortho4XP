...
def build_overlay(lat, lon):
    ...
    file_to_sniff = os.path.join(
        custom_overlay_src, "Earth nav data", FNAMES.long_latlon(lat, lon) + ".dsf"
    )
    if not os.path.exists(file_to_sniff):
        UI.exit_message_and_bottom_line(
            "   ERROR: file ",
            file_to_sniff,
            "absent. Recall that the overlay source directory needs to be set in the config window first.",
        )
        return 0
    ...
    try:
        ...
        UI.exit_message_and_bottom_line(
            "   ERROR: could not copy it. Disk full, write permissions, erased tmp dir ?"
        )
        return 0
    ...
    if dsfid == "7z":
        ...
        os.system(
            unzip_cmd + " e -o" + FNAMES.Tmp_dir + ' "' + file_to_sniff_loc + '.7z"'
        )
    ...
    dsfconvertcmd = [
        ...
        os.path.join(
            FNAMES.Tmp_dir, FNAMES.short_latlon(lat, lon) + "_tmp_dsf_without_mesh.dsf"
        ),
    ]
    fingers_crossed = subprocess.Popen(dsfconvertcmd, stdout=subprocess.PIPE, bufsize=0)
    while True:
