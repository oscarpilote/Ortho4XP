...
class Ortho4XP_Custom_ZL(tk.Toplevel):
    ...
    def __init__(self, parent, lat, lon):
        ...
        tk.Label(self.frame_left, anchor=W, text="Zoomlevel : ", bg="light green").grid(
            row=row, column=0, padx=5, pady=3, sticky=W
        )
        self.zl_combo = ttk.Combobox(
            self.frame_left,
        ...
        ttk.Button(
            self.frame_left, text="Preview", command=lambda: self.preview_tile(lat, lon)
        ...
        tk.Label(
            self.frame_left, anchor=W, text="Approx. Add. Size : ", bg="light green"
        ...
        row += 1
        ttk.Button(self.frame_left, text="    Exit     ", command=self.destroy).grid(
            row=row, column=0, padx=5, pady=3, sticky=N + S + E + W
        )
        row += 1
