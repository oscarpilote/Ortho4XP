...
def sort_and_reconstruct_runways(tile, airport_layer, dico_airports):
    ...
    for airport in dico_airports:
        ...
        runway_area = VECT.ensure_MultiPolygon(
            ops.cascaded_union([item[0] for item in runways_as_area + runways_as_line])
        )
...
def update_airport_boundaries(tile, dico_airports):
    ...
    try:
        ...
        UI.vprint(
            1, "WARNING: Could not save airport info to file", FNAMES.apt_file(tile)
        )
    return
...
def smooth_raster_over_airports(tile, dico_airports, preserve_boundary=True):
    ...
    for airport in dico_airports:
        ...
        airport_im = Image.new(
            "L", (upscale * (colmax - colmin + 1), upscale * (rowmax - rowmin + 1))
        )
    ...
    for airport in dico_airports:
        ...
        for (
            ...
            for pol in VECT.ensure_MultiPolygon(VECT.cut_to_tile(runway_pol)):
                way = numpy.round(
                    VECT.refine_way(numpy.array(pol.exterior.coords), refine_size), 7
                )
