...
a particular server.",
    ...
    "limit_tris": {
        "type": int,
        "default": 0,
        "hint": "If non zero, upper bound on the number of final triangles in the mesh.",
    },
    "hmin": {
        "type": float,
        "default": 0,
        "hint": "The mesh algorithm will not try to subdivide triangles whose shortest edge is already smaller than hmin (in meters). If hmin is smaller than half of the levation data step size, it will default to it anyhow (its default zero value thus means : as good as the DEM can do).",
    },
    "min_angle": {
        "type": float,
        "default": 10,
        "hint": "The mesh algorithm will try to not have mesh triangles with second smallest angle less than the value (in deg) of min_angle (prior to v1.3 it was the smallest, not second smallest) The goal behind this is to avoid potential artifacts when a triangle vertex is very close the the middle of its facing edge.",
    },
    "sea_smoothing_mode": {
        "type": str,
        "default": "zero",
