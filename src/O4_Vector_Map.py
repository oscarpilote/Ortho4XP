...
def include_roads(vector_map, tile, apt_array, apt_area):
    ...
    if tile.road_level >= 2:
        ...
        timer = time.time()
        (road_network_banked_2, road_network_flat_2) = OSM.OSM_to_MultiLineString(
            road_layer, tile.lat, tile.lon, tags_for_exclusion, road_is_too_much_banked
        )
    ...
    if not road_network_flat.is_empty:
        ...
        UI.vprint(
            1, "    * Encoding the remaining primary road network as linestrings."
        )
        vector_map.encode_MultiLineString(
...
def include_sea(vector_map, tile):
    ...
    if os.path.isfile(custom_coastline):
        ...
        UI.vprint(
            1, "    * User defined custom coastline data detected (multiple files)."
        )
    ...
    for pfile_name in os.listdir(patch_dir):
        ...
        try:
            patch_layer.update_dicosm(
                os.path.join(patch_dir, pfile_name), input_tags=None, target_tags=None
            )
        except:
...
#############################################################################
def keep_obj8(
    lat_anchor, lon_anchor, alt_anchor, heading_anchor, objfile_name, vector_map, tile
):
    dico_idx_nodes = {}
    idx_node = 0
    dico_index = {}
