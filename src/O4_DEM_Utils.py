...
available_sources = (
    ...
    "ALOS",
    "ALOS 3W30 average (from OpenTopography) - mostly worldwide",
)
...
def ensure_elevation(source, lat, lon, verbose=True):
    if source == "View":
        ...
        url_base = (
            "https://prd-tnm.s3.amazonaws.com/StagedProducts/Elevation/1/ArcGrid/"
        )
        usgs_base = "n" + str(lat + 1) + "w" + str(-lon).zfill(3)
        r = http_request(
            url_base + "USGS_NED_1_" + usgs_base + "_ArcGrid.zip", source, verbose
        )
        if not r:
            UI.vprint(2, "    Trying alternative naming scheme.")
        ...
        with zipfile.ZipFile(io.BytesIO(r.content), "r") as zip_ref:
            ...
            for f in zip_ref.filelist:
                ...
                with open(
                    os.path.join(
                        os.path.dirname(FNAMES.elevation_data(source, lat, lon)), fname
                    ),
                    "wb",
                ) as out:
                    try:
