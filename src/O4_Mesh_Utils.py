...
def build_curv_tol_weight_map(tile, weight_array):
    ...
    if tile.coast_curv_tol != tile.curvature_tol:
        UI.vprint(
            1, "-> Modifying curv_tol weight map according to coastline location."
        )
...
def write_mesh_file(tile, vertices):
    ...
    for i in range(0, nbr_vert):
        f.write(
            "{:.7f}".format(vertices[6 * i] + tile.lon)
            + " "
            + "{:.7f}".format(vertices[6 * i + 1] + tile.lat)
            + " "
            + "{:.7f}".format(vertices[6 * i + 2] / 100000)
            + " 0\n"
        )
    f.write("\n")
    f.write("Normals\n")
...
def build_mesh(tile):
    ...
    if not tile.iterate:
        ...
        try:
            ...
            if (
                ...
                tile.dem = DEM.DEM(
                    tile.lat, tile.lon, source, fill_nodata=False, info_only=False
                )
    ...
    hmin_effective = max(
        tile.hmin, (tile.dem.y1 - tile.dem.y0) * GEO.lat_to_m / tile.dem.nydem / 2
    )
    mesh_cmd = [
