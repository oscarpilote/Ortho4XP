...
class Vector_Map:
    ...
    def __init__(self):
        self.dico_nodes = (
            {}
        )  # keys are tuples of 2 floats (in our case (lon-base_lon), lat-base_lat) and values are ints (ids)
        self.dico_edges = (
            {}
        )  # keys are tuples of 2 ints (end-points ids) and values are ints (ids). An egde id is needed for the index (bbox)
        ...
        self.data_nodes = (
            {}
        )  # keys are ints (ids) and values are floats (vector altitude)  # could easily be upgraded to arrays if necessary
    ...
    def insert_edge(self, id0, id1, marker, check=True):
        ...
        weight_list = (
            []
        )  # affine coordinates of points in between pts id0 and id1 that belong to existing edges
                ...
                for i in range(1, 3):
                    if ordered_data[i][0] > 0 and ordered_data[i][0] < 1:
                        ...
                        if ordered_data[i + 1][0] < 1:
                            self.create_edge(
                                ordered_data[i + 1][1], ordered_data[i + 2][1], c_marker
                            )
                        break
    ...
    def are_encroached(self, a, b, c, d):
        ...
        if abs(numpy.linalg.det(A)) > eps:
            ...
            alpha0, alpha1 = (a - c)[g_idx] / (a - b)[g_idx], (a - d)[g_idx] / (a - b)[
                g_idx
            ]
            beta0, beta1 = (c - a)[d_idx] / (c - d)[d_idx], (c - b)[d_idx] / (c - d)[
                d_idx
            ]
            return (
...
def coastline_to_MultiPolygon(coastline, lat, lon, custom_source=False):
    ...
    for line in coastline.geoms:
        if line.is_ring:
            ...
            if (
                numpy.min(
                    numpy.abs(
                        [tmp[-1][0] - int(tmp[-1][0]), tmp[-1][1] - int(tmp[-1][1])]
                    )
                )
                > 0.00001
            ):
    ...
    if osm_error:
        UI.lvprint(
            1, "ERROR in OSM coastline data. Coastline abruptly stops at", osm_badpoints
        )
        return geometry.MultiPolygon()
...
####################################################################################################
def improved_buffer(
    input_geometry, buffer_width, separation_width, simplify_length, show_progress=False
):
...
def convolve_periodic(way, kernel):
    ...
    return numpy.convolve(
        numpy.concatenate((way[-k - 1 : -1], way, way[1 : k + 1])), kernel, "valid"
    )
