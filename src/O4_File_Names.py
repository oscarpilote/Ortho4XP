...
def custom_water(lat, lon):
    return os.path.join(
        OSM_dir, long_latlon(lat, lon), short_latlon(lat, lon) + "_custom_water.osm.bz2"
    )
...
def elevation_data(source, lat, lon):
    if source == "View":
        ...
        return os.path.join(
            Elevation_dir, long_latlon(lat, lon) + "_NED1", "w001001.adf"
        )
...
def jpeg_file_dir_from_attributes(lat, lon, zoomlevel, provider):
    if not provider:
        ...
        file_dir = os.path.join(
            Imagery_dir, short_latlon(lat, lon), provider["code"] + "_" + str(zoomlevel)
        )
    elif provider["imagery_dir"] == "grouped":
        file_dir = os.path.join(
            Imagery_dir, long_latlon(lat, lon), provider["code"] + "_" + str(zoomlevel)
        )
    elif provider["imagery_dir"] == "code":
        file_dir = os.path.join(
            Imagery_dir, provider["code"], provider["code"] + "_" + str(zoomlevel)
        )
    else:
        file_dir = os.path.join(
            Imagery_dir,
