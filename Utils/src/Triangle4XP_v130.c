...
struct badtriang {
	...
	/*REAL key;                           /* cos^2 of smallest (apical)
	 * angle. */
...
void enqueuebadtri(struct mesh *m, struct behavior *b, struct otri *enqtri,
	...
	#else  /* not ANSI_DECLARATORS */
void enqueuebadtri(m, b, enqtri, priority, enqapex, enqorg,
...

{
	...
	if (b->usertest) {
		...
		priority = triunsuitable(torg, tdest, tapex,
					 elemattribute(*testtri, 0));
		if (priority) {
			enqueuebadtri(m, b, testtri, priority, tapex, torg,
				      tdest);
			return;
		}
	}
